...
from cobaya.conventions import _output_prefix, _debug, _debug_file, _force_reproducible
...
#        renames_flat = [set([k] + str_to_list(v)) for k, v in renames.items()]
        renames_flat = [set([k] + str_to_list(v)) for k, v in getattr(renames, "items", lambda: [])()]
